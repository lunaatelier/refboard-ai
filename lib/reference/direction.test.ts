...
import type { SearchedImageLike } from "./direction";
...
describe("buildDirectionOptions — 무드+팔레트+이미지 조립", () => {
  ...
  it("이미지 후보는 최대 6장, 앞 4장만 기본 선택된다", () => {
    const images = Array.from({ length: 8 }, (_, i) =>
      makeImage(`https://img/${i}.jpg`, "unsplash", `photographer${i}`),
    );
  ...
  it("첫 이미지는 hero, 다음 2~3장은 supporting, 나머지는 detail", () => {
    const images = Array.from({ length: 5 }, (_, i) => makeImage(`u${i}`, "pexels", `a${i}`));
    const directions = buildDirectionOptions(
      [makeMood("m1", "trust")],
      paletteOptions,
      { m1: images },
  ...
  it("이미지 순서(order)는 원본 배열 순서를 그대로 보존한다", () => {
    const images = [makeImage("u0", "pexels", "a0"), makeImage("u1", "pexels", "a1")];
    const directions = buildDirectionOptions(
      [makeMood("m1", "trust")],
      paletteOptions,
      { m1: images },
...
describe("이미지 상세 조작 (P3-4) — 선택/역할/순서/교체/재생성", () => {
  ...
  function makeDirection(count: number): DirectionOption {
    const images = Array.from({ length: count }, (_, i) => makeImage(`u${i}`, "pexels", `a${i}`));
  ...
  describe("replaceImageCandidate", () => {
    it("url/source/attribution만 바뀌고 role/selected/order는 유지된다", () => {
      ...
      const next = replaceImageCandidate(d, "u0", makeImage("new-url", "unsplash", "new photographer"));
  ...
  describe("applyRegeneratedImages", () => {
    it("이미지 세트 전체를 교체하고 기본 role/selected를 다시 적용한다", () => {
      ...
      const next = applyRegeneratedImages(d, [
        makeImage("v0", "unsplash", "p0"),
        makeImage("v1", "unsplash", "p1"),
      ]);
