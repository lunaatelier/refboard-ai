...
describe("parseWorkerCore — pdf 분기가 parsePdfDocument와 동일하게 동작한다", () => {
  it("유효한 pdf는 워커 경로와 서버 경로가 완전히 동일한 결과를 낸다", async () => {
    // unpdf(pdf.js)는 입력 버퍼를 내부적으로 transfer해 소비한다 — 같은
    // ArrayBuffer를 두 번째 파싱 호출에 재사용하면 "detached ArrayBuffer"로
    // 실패한다(실측 확인). 실제 Worker 흐름에서도 버퍼당 파싱은 항상 1회뿐이라
    // 문제 없지만, 이 테스트는 두 경로를 각각 독립적으로 호출해야 하므로 매번
    // 새 버퍼를 만든다.
    const text = "Worker vs server equivalence";
    const viaWorker = await parseInWorker("pdf", buildSamplePdf(text));
    const viaServer = await parsePdfDocument(buildSamplePdf(text));
    assert.deepEqual(viaWorker, viaServer);
    assert.match(viaWorker.text, /Worker vs server equivalence/);
  });
