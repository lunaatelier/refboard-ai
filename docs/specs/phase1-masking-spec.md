...
│  ├─ api/
│  │  └─ parse/route.ts             # 서버 파싱 (pdf/pptx만, 메모리·무저장)
│  └─ layout.tsx
├─ components/
│  ├─ shell/
...
### 7.2 서버 파싱 — pdf/pptx (`app/api/parse/route.ts`)
파싱 라이브러리가 무거워 서버 처리가 불가피한 포맷만 해당.
```
POST /api/parse  (multipart/form-data, file)
처리:
  - 확장자 분기: pdf → pdf-parse / pptx → 텍스트 추출
  - 추출 텍스트를 JSON으로 반환 { text: string }
보안 (반드시 준수):
  - 추출 텍스트·파일 내용을 로그에 남기지 않는다.
  - 외부(Gemini/Unsplash/Pexels)로 전송하지 않는다. 이 라우트는 파싱만 담당.
  - 파일은 메모리에서만 처리하고 디스크·DB에 영구 저장하지 않는다.
  - 응답은 추출 텍스트만. 처리 후 서버 메모리에서 해제.
```
라이브러리: `pdf-parse`(PDF), pptx는 unzip 후 slide XML 텍스트 추출 또는 파서 라이브러리 — 구현 시 가용성 확인 후 선택.
