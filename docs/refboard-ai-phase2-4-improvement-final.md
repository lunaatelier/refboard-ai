...
- 실제 `maskedContent`와 팔레트가 전달돼도 `카드 1/2/3`, `지표 1/2/3` 같은 플레이스홀더가 남는다. `table` 렌더러도 동일하게 고정 셀 문자열을 그린다(P0 코드 재확인으로 확정 — `hero`/`timeline` 렌더러만 `maskedContent`를 실제로 파싱한다).
...
5. ~~기존 지시서의 `A1~A6` 오기를 `A1~A3` 또는 실제 검증 항목 수로 정정한다.~~ P0 검증 결과 저장소 어디에도(작업 트래커·기존 스펙 문서) `A1~A6` 원본 참조를 찾지 못했다. 이 저장소는 검증 항목을 "게이트 1/2" 표기로 관리하므로, 존재하지 않는 A번호 체계는 폐기하고 이후 구현·테스트는 게이트 1/2 또는 본 문서의 P0~P10 기준으로만 관리한다.
...
7. **업로드 구조 확정 (P0 결정):** pdf·pptx 모두 브라우저 파싱으로 이관한다. Vercel Function 요청 본문 한도(약 4.5MB)에서는 20MB든 200MB든 실제로 보장되지 않았는데, `unpdf`(edge/브라우저 호환)와 `jszip`(isomorphic) 둘 다 브라우저에서 그대로 동작해 이 제약 자체가 구조적으로 사라진다. CLAUDE.md §4.2("pdf/pptx는 파싱 라이브러리가 무거워 서버에서 파싱")의 전제가 더 이상 성립하지 않으므로 §4.2도 함께 갱신한다(아래 완료 기준 참고).

   롤아웃 순서(한 번에 스위치하지 않는다):
   1. `lib/parse/parseDocumentLocally.ts`(가칭) 클라이언트 파싱 진입점을 추가한다. pdf/pptx 파서는 동적 import로 불러 초기 번들 크기를 늘리지 않는다.
   2. 파싱은 Web Worker에서 실행해 큰 파일이 UI를 멈추지 않게 한다.
   3. pptx는 압축 파일이므로 업로드 파일 크기뿐 아니라 **압축 해제 후 크기와 파일 개수 상한**을 둔다(zip bomb 방어).
   4. 기존 `/api/parse`·`parseViaServer` 경로와 신규 브라우저 파서의 결과를 동등성 테스트로 비교한다.
   5. 대용량·손상 파일·압축 폭탄 케이스를 방어 테스트한 뒤 클라이언트 경로로 전환한다.
   6. 동등성 확인 후에만 `/api/parse`를 제거한다. 그 전까지는 남겨두고, 200MB 변경은 커밋하지 않는다(§11 커밋 0).
   7. 200MB 같은 임의로 큰 한도를 바로 선언하지 않는다. 초기 한도는 보수적으로 유지하고, 실제 브라우저 메모리 테스트 결과로 조정한다.
...
- pdf·pptx 브라우저 파싱 이관이 확정되고, `/api/parse` 제거는 동등성 테스트 통과 후로 순서가 명시된다. 그 전까지 200MB 변경은 커밋되지 않는다.
- CLAUDE.md §4.2의 "pdf/pptx는 서버에서만 파싱" 문구가 갱신된다.
...
12. **(P9-A 완료 후 진행 — 순서 의존성 확인됨)** §6.7의 온디맨드 콘텐츠 매핑 생성 함수와 `conceptOptionId + contentVariantId + briefHash` 캐시를 구현한다. `/api/concept`가 `ConfirmedReferenceBrief`를 실제로 받기 전까지는 `briefHash`가 의미 있는 캐시 키가 되지 않으므로, P9-A(위 §P9 "착수 순서 재확정" 참고)를 먼저 마친다.
...
1. 디자인 검색어는 다음 3개 의도 축으로 **로컬 규칙만으로 즉시** 생성한다(AI 호출 없음).
   - 패턴축: 섹션의 `layoutPattern`, `contentType` 중심
   - 무드축: 확정된 글로벌 방향의 무드 중심
   - 업종축: 프로젝트 `domain`, `businessDomains` 중심
...
### 기존 `section-queries` route 재활용 (P0 검증 결과 반영)

P0 코드 재확인에서 `app/api/section-queries/route.ts`와 `lib/reference/platforms.ts`(`PlatformKeywordProfile`, `validateKeywordForPlatform`, `buildProfiledPlatformQueries`)가 플랫폼별 언어·어휘 규칙을 이미 구현하고 있다는 것을 확인했다. 이 코드는 폐기하지 않고 역할을 바꾼다.

- 기본 검색어(위 1번의 3축)는 **로컬 생성만으로 화면에 즉시 표시**되고, 이 단계에는 AI 호출이 없다.
- 기존 `section-queries` route는 "기본 생성기"에서 **선택적 `AI로 검색어 다듬기` 향상 단계**로 강등한다. 사용자가 명시적으로 눌렀을 때만 호출한다.
- 호출은 섹션 1개당 1회가 아니라, **고영향 섹션 전체를 한 번에 배치 처리**한다.
- 같은 `directionHash + 섹션 목록 + promptVersion` 조합은 캐시하고 재호출하지 않는다.
- `PlatformKeywordProfile`의 언어·어휘·검증 로직은 그대로 재사용한다 — 다듬기 단계가 호출될 때 이 로직이 적용된다.

```text
로컬 기본 intent(3축, AI 0회)
    ↓ 사용자가 확인·수정, 바로 검색 가능
    ↓ (선택) "AI로 검색어 다듬기" 버튼
기존 section-queries route — 고영향 섹션 배치, directionHash 캐시
    ↓
플랫폼 특성에 맞는 표현으로 보정된 검색어
```

이렇게 해야 §3.4(결정론적 조합 우선)와 §P4 호출 예산 표("섹션 검색어: 로컬 intent 조합, AI 0회, 선택적 향상 1회")가 실제 구현과 일치한다. 기존 route를 기본 생성기로 유지하면 레퍼런스 화면에 들어갈 때마다 Gemini 가용성에 의존하게 되어 §P10의 "부분 실패해도 진행 가능" 원칙과 충돌한다.
...
- 섹션 진입 시 기본 검색어가 AI 호출 없이 즉시 표시되고, 기존 `section-queries` route는 사용자가 "AI로 다듬기"를 눌렀을 때만 고영향 섹션 배치로 호출된다.
...
### 착수 순서 재확정 — P9-A/P9-B 분리 (P1 구현 중 발견된 의존성 반영)

P1을 실제로 구현하다가 §6.7(온디맨드 콘텐츠 매핑, item 12)이 이 P9 전체를 전제로 한다는 것을 확인했다: 온디맨드 매핑의 캐시 키(`briefHash + conceptOptionId + contentVariantId`)는 `/api/concept`가 실제로 `ConfirmedReferenceBrief`를 받아야만 의미가 생기고, "기준 변형으로 구조 3안 생성"이라는 흐름 자체가 이 아래 §P9의 3축(구조·신뢰형/비주얼·몰입형/균형형) 차별화를 전제로 한다. 그래서 P9를 다음 두 조각으로 나눠 **P9-A를 먼저, P1 item 12보다 앞서** 진행한다.

- **P9-A (계약 부분, 먼저 진행):**
  - `/api/concept`가 개별 필드(`paletteOption`/`moodKeywords`/`layoutBySection`/`targetImplications`) 대신 `ConfirmedReferenceBrief`를 받도록 변경(레거시 입력은 폐기하거나 adapter로 흡수).
  - 구조·신뢰형/비주얼·몰입형/균형형을 명시적 enum으로 고정(아래 작업 10~12).
  - 신규 생성 결과에 `version: "2.0"` + `sourceBasis` 기록, builder는 이 경우 `sourceBasis`/`briefHash`가 없으면 생성을 거부한다(§6.3 버전 강제 규칙과 연결).
  - `useVariants`가 디자인 3안의 차별화 축을 결정하지 않도록 완전히 분리(§6.7 온디맨드 규칙으로 이관, 작업 14).
  - **완료 기준:** `briefHash + conceptOptionId + contentVariantId + promptVersion`을 캐시 키로 쓸 수 있는 상태가 된다 → 이 시점에 보류했던 P1 item 12(온디맨드 콘텐츠 매핑)를 진행한다.
- **P9-B (프리뷰/렌더러 부분, 나중에 진행):** 아래 작업 7~9(`ConceptPreview`의 layout renderer를 실제 콘텐츠 기반으로 교체, 플레이스홀더 제거)는 P9-A와 분리해 이후 별도로 진행한다.

실행 순서: **P1 핵심(완료) → P9-A → P1 item 12(온디맨드 콘텐츠 매핑) → P9-B.**
...
9. `카드 1/2/3`, `지표 1/2/3` 문자열과 `table` 렌더러의 고정 셀 문자열을 제거한다.
...
**P0 코드 재확인 결과:** 현재 `app/api/target-analyze/route.ts`(브랜드 심층 분석 최대 3개), `app/api/generate-image/route.ts`(이미지 생성 최대 3장), `components/reference/TargetsTab.tsx`(분석 대상 채택 개수) 어디에도 상한을 강제하는 코드가 없다 — 클라이언트가 버튼을 누르는 횟수만큼 그대로 호출된다. 즉 P4 호출 예산 표의 상한은 현재 문서상의 목표치일 뿐 실제로 강제되는 곳이 없다. 이 섹션의 작업이 새로 만드는 게 아니라 **처음으로 도입하는** 것임을 분명히 한다.
