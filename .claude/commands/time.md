...
## 5. 이번 세션 작업 요약 — 비개발자 친화적 모드 (필수)

현재 대화 내용을 바탕으로 이번 세션에서 수행한 작업을 요약합니다. 민감 정보(전체 경로, 계정명 등)는 마스킹합니다.

작성 원칙 (팀장·기획자 등 비개발자가 읽어도 이해할 수 있어야 한다):

- **개발 용어를 그대로 쓰지 않는다.** 기술 용어(API, 리팩터링, 정규화, SSoT, 파이프라인, 아키텍처, grounding 등)는 그 자체로 쓰지 말고 "무엇을 할 수 있게 됐는지" 결과 중심으로 풀어 쓴다.
  - 나쁜 예: "Gemini 분석(슬라이드 계보·기존시안 감지)·전역지시 전파·이미지 opt-in 구현"
  - 좋은 예: "AI가 문서를 읽고 어떤 프로젝트인지, 어떤 화면들로 구성됐는지 자동으로 분석하는 기능을 만듦. 문서 속 이미지도 사용자가 동의한 것만 분석하도록 함(기본은 텍스트만, 안전 우선)"
- **맨 앞에 한 줄 요약을 둔다**: 이번 세션에서 "무엇이 가능해졌는지"를 한 문장으로.
- **구현 단계(Step)가 있는 프로젝트라면, 완료한 단계를 순서대로 한 줄씩 결과 중심으로 풀어 쓴다** (구현 방법이 아니라 "무엇이 가능해졌는지"로 끝난다).
- **환경/도구 이슈 등은 있었다는 사실만 짧게 적고 기술적 원인은 생략한다.**
...
**"총 H시간 M분"은 세션 시작~종료 시각의 순수 경과 시간**(첫 사용자 메시지 ~ 마지막 사용자 메시지, IDLE_CAP 미적용)입니다. `AI 협업 사람 능동시간`(자리비움 10분 캡 적용)과는 다른 값이니 혼동하지 않습니다. 60분 미만이면 "M분"만 표기(예: 43분). 자정을 넘긴 세션은 날짜 차이를 반영해 계산합니다(예: 19:49~다음날 09:12 → 13시간 23분).
...
오늘이 **목요일**이면 주간 합계를 파일 끝에 추가합니다. 계산 기준(중요):

1. **Claude Code 세션 전부**의 `AI 협업 사람 능동시간`과 `세션 시간(총 경과시간)`을 각각 합산합니다 — 이게 기준선입니다.
2. **각 Codex 세션**에 대해, 그 세션의 시작~종료 시각 구간이 **어느 Claude Code 세션의 구간과도 겹치지 않는지** 확인합니다.
   - 겹치지 않으면: 그 Codex 세션의 능동시간·세션시간을 1번 합계에 **더합니다**.
   - 하나라도 겹치면: 그 Codex 세션은 합계에서 **제외**합니다(같은 사람이 동시에 두 도구에서 능동적으로 작업할 수 없으므로 중복 계산 방지).
3. Codex 세션끼리 겹치는 경우나 Claude Code 세션끼리 겹치는 경우는 이 규칙의 대상이 아닙니다(각자 그대로 합산).
