...
**구현 현황 (2026-07-24 실측 갱신):** `implementation-steps.md`의 Step 1~19는 전부 구현 완료 상태다(이후 `docs/refboard-ai-phase2-4-improvement-final.md` P0~P10 개선 사이클도 완료). §6 Phase 표가 실제 구현을 따라가지 못해 오래 "착수/대기"로 남아있었던 적이 있으니, 새 세션에서 "지금 뭐가 됐는지" 판단할 땐 이 문서의 상태 라벨보다 **실제 코드/실브라우저 확인을 우선**한다. 아래 §6 표가 그 실측 결과다.
...
| NVIDIA NIM | **이미지 실제 생성**(키비주얼·로그인 이미지) | **발급·사용 중 (Step 19).** 기본 모델 `black-forest-labs/flux.1-dev`, `NVIDIA_IMAGE_MODEL`로 교체 가능. 키 없으면 프롬프트 표출만(자동 폴백) |
...
# 이미지 실제 생성 (Step 19) — NVIDIA NIM. 없으면 프롬프트 표출만 (자동 폴백)
NVIDIA_API_KEY=
# NVIDIA_IMAGE_MODEL=black-forest-labs/flux.1-dev  # 기본값, 교체 시에만 설정
```
...
- **OCR 선마스킹 (Step 18 구현됨):** 전송 동의 전에 브라우저(tesseract.js WASM, kor+eng)로 이미지 텍스트를 읽어 마스킹 엔진으로 검사한다. 민감어 감지 이미지는 자동 체크 해제(기본 제외) + 종류별 개수 배지 표시. OCR 원문은 요약(종류·개수)만 남기고 즉시 폐기 — 이미지·원문 모두 외부로 나가지 않는다. 엔진·언어 데이터는 최초 1회 CDN 다운로드(다운로드만 있고 업로드 없음).
...
> **상태 갱신 이력:** 2026-07-24, 실제 앱을 처음부터 끝까지 라이브로 돌려(가짜 프로젝트 업로드 → 마스킹 → 실제 Gemini 분석 → 실제 Gemini+Unsplash/Pexels 레퍼런스·무드 → 실제 Gemini 컨셉 3안 생성까지) 검증 후 갱신. 이전엔 이 표가 "Phase 1 착수"로 오래 멈춰 있어 실제 구현 범위를 계속 과소평가하게 만들었다 — 표 상태는 손으로 갱신해야 하므로 개발이 빠르게 진행되면 뒤처지기 쉽다는 점 주의.

| Phase | 책임 (한 동사) | 범위 | 산출물 | 상태 |
| --- | --- | --- | --- | --- |
| Phase 1 | 셸+마스킹 | 랜딩 업로드 + 워크스페이스 셸 + LNB 스텝퍼 + 3계층 마스킹(파싱→탐지→검수→미리보기), pdf/pptx 브라우저 Worker 파싱 포함 | 동작하는 셸 + 마스킹 게이트 | **완료** |
