...
tokens:
  ...
  components:
    button-fill-primary: { type: button, bg: "#6366F1", fg: "#ffffff", radius: 8, padding: "0 16px", font: "14/600", use: "주요 CTA — 다음 단계, 확정하기. 데스크톱 기본 40px 높이" }
    button-fill-neutral: { type: button, bg: "#334155", fg: "#ffffff", radius: 8, padding: "0 16px", font: "14/600", use: "강한 보조 액션 — 원문 폐기, 사전 등록" }
    button-fill-danger: { type: button, bg: "#EF4444", fg: "#ffffff", radius: 8, padding: "0 16px", font: "14/600", use: "파괴적 확인 — 프로젝트 삭제, 검수 항목 전체 해제" }
...
- Use: 주 CTA — "다음 단계", "마스킹 확정", "컨셉 확정" (40px 높이)
...
- Radius: 8px / Padding: 0 16px / Font: 14px / 600
- Use: 파괴적 확인 — "프로젝트 삭제", "검수 항목 전체 해제"
...
- Radius: 8px / Padding: 0 16px / Font: 14px / 600
- Use: 약한 파괴적 액션 — "검수에서 제외" (완전 삭제 아님)
...
- 항상 워크스페이스 좌측에 고정 노출(240px), 스크롤 시에도 sticky

---

**Verified:** 2026-07-06 (내부 확정, 외부 소스 없음)
...
- LNB: 240px 고정 폭 / 작업 영역: 유동 폭, 최대 960px 중앙 정렬 (컨셉 비교 화면은 최대 1200px까지 확장)
...
|------|-------|-------------|
| Desktop (Primary) | ≥1024px | 전체 디자인 충실도, LNB 고정 240px |
| Tablet | 768–1023px | LNB 아이콘 전용으로 축소, 작업 영역 우선 |
| Mobile (보조) | <768px | LNB가 상단 가로 스텝바로 전환. 1차 지원 타겟 아님 — 열람 위주 |
