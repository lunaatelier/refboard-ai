...
- **구조(고정 순서):** `banner`(선택) → `title`(22px/700) + `description`(선택, 16px/`--text-muted`) → `children`(화면 고유 카드 스택) → `cta`(선택, 카드 밖·맨 아래·좌측 정렬). 전체 세로 gap은 `--space-base`.
...
- **`PageCta`**: 하단 CTA 버튼의 표준 컴포넌트. 화면마다 10px/12px/14px, 600/700, 14px/16px로 흩어져 있던 값을 `padding: 10px var(--space-base)`, `14px/600`으로 통일. `locked` prop이 true면 `--locked` 배경(완료/비활성 상태 표현), 기본은 `.btn-primary`.
